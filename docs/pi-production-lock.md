...
| Pi 受控 fork              | `blueforst/pi` baseline `ab5f8d88…`；seam commit `5e93510b…`（tree `5a16381b…`，issue iris_agent#41 修复链 HEAD）；acceptedRuntime `e209e561…`（tree `771b1634…`，与 Pi 权威 lock 一致）；upstream base `e741cb05…`；audit baseline `b4f2936…/0.82.1` | 已锁定（file link 本地开发，未来多机部署切换 tarball/npm 发布）  |
